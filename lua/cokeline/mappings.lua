...
local by_index = function(goal, index)
  local target_buffer = vim_filter(
    function(buffer) return buffer.index == index end,
    _G.cokeline.visible_buffers
  )[1]
  if not target_buffer then return end
...
local by_step = function(cmd, step)
  ...
  local target_buffer =
    ...
    or (gl_settings.cycle_prev_next and _G.cokeline.valid_buffers
          [(target_valid_index - 1) % #_G.cokeline.valid_buffers + 1])
    or nil
  if not target_buffer then return end

  local _ =
    (cmd == 'switch' and buffers.move_buffer(current_buffer, target_buffer._valid_index))
...
local pick = function(cmd)
  ...
  local _ =
    (cmd == 'focus' and vim_cmd('b' .. target_buffer.number))
    or (cmd == 'close' and vim_cmd(('bd%s | redrawtabline'):format(target_buffer.number)))
end
