...
local get_sidebar_components = function(sidebar, winid, default_hl)
  ...
  if vim.bo[bufnr].filetype ~= sidebar.filetype then return {} end
  ...
  local components = vim_map(function(comp)
    return rq_components.comp_to_component(comp, default_hl, {number = bufnr})
  ...
  elseif width_of_components < sidebar_width then
    ...
    components[#components].text =
      components[#components].text .. str_rep(' ', space_left)

    components[#components].width =
      components[#components].width + space_left
  end
...
local get_left_right_sidebar_components =
  ...
  local left_sidebar_components =
    (first_split[1] == 'leaf' and left_sidebar)
    and get_sidebar_components(left_sidebar, first_split[2], default_hl)
     or {}

  local right_sidebar_components =
    (last_split[1] == 'leaf' and right_sidebar)
    and get_sidebar_components(right_sidebar, last_split[2], default_hl)
     or {}
